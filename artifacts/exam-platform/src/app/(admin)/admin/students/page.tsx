...
async function fetchStudents(page: number): Promise<{ data: StudentStat[]; pagination: { page: number; total: number; totalPages: number; limit: number } }> {
  const res = await fetch(`/api/admin/students?page=${page}&limit=20`);
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-0.5">{data?.pagination.total ?? "–"} active students</p>
      </div>

      {isLoading ? (
        <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-16 bg-gray-100 animate-pulse rounded-lg" />)}</div>
      ) : (
        <>
          {data?.data.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 text-left">
                      ...
                      <th className="px-4 py-3 font-medium text-gray-600">Avg Score</th>
                      <th className="px-4 py-3 font-medium text-gray-600">Total Score</th>
                      <th className="px-4 py-3 font-medium text-gray-600">Passed</th>
                      <th className="px-4 py-3 font-medium text-gray-600">Last Active</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={s.userId} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-xs text-gray-600 truncate max-w-[120px]">{s.userId}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 font-medium">{s.totalAttempts}</td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 font-semibold text-violet-600">{s.totalScore}</td>
                        <td className="px-4 py-3">
