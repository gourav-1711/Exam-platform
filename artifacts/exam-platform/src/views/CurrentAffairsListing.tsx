    ...
    <PageTransition className="p-4 md:p-8 max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">Daily news analysis and updates for competitive exams.</p>
        </div>
        
        <div className="relative w-full md:w-72 shrink-0">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input placeholder="Search articles..." className="pl-9 bg-card border-border/50 rounded-xl" />
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {isLoading ? (
          ...
          data?.data.map((article) => (
            <Card key={article.id} className="card-hover border-border/50 rounded-2xl bg-card overflow-hidden flex flex-col">
              <CardContent className="p-5 flex flex-col flex-1">
                ...
                <Link href={`/current-affairs/${article.id}`}>
                  <Button variant="ghost" className="w-full justify-between px-0 hover:bg-transparent hover:text-primary">
