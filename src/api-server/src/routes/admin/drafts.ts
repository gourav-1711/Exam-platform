...
router.get("/drafts/:id", async (req, res) => {
  try {
    ...
    const [draft] = await db
      ...
      .from(draftsTable)
      .where(
        and(eq(draftsTable.id, id), eq(draftsTable.createdBy, auth.userId!)),
      );
...
router.post(
  "/drafts",
  logAdminActivity("create_draft", "draft"),
  async (req, res) => {
    try {
      const auth = getAuth(req);
      ...
      if (!resourceType || !content) {
        return res
          .status(400)
          .json({ error: "resourceType and content are required" });
      }
...
router.patch(
  "/drafts/:id",
  logAdminActivity("update_draft", "draft"),
  async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      const auth = getAuth(req);
...
router.delete(
  "/drafts/:id",
  logAdminActivity("delete_draft", "draft"),
  async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      const auth = getAuth(req);
