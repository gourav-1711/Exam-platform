...
router.get("/exams/:id", async (req, res) => {
  try {
    ...
    const examQs = await db
      ...
      .leftJoin(
        questionsTable,
        eq(examQuestionsTable.questionId, questionsTable.id),
      )
...
router.post(
  "/exams",
  examCreationLimiter,
  logAdminActivity("create_exam", "exam"),
  async (req, res) => {
    try {
      ...
      const [exam] = await db
        .insert(examsTable)
        .values({ ...parsed.data, createdBy: auth.userId ?? null })
        .returning();
...
router.patch(
  "/exams/:id",
  logAdminActivity("update_exam", "exam"),
  async (req, res) => {
    try {
      const id = routeParamInt(req.params.id);
...
router.delete(
  "/exams/:id",
  logAdminActivity("delete_exam", "exam"),
  async (req, res) => {
    try {
      const id = routeParamInt(req.params.id);
      await db
