...
router.get("/study-notes/:id", async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const [note] = await db.select().from(studyNotesTable).where(eq(studyNotesTable.id, id));
...
router.post("/study-notes", logAdminActivity("create_study_note", "study_note"), async (req, res) => {
  try {
...
router.patch("/study-notes/:id", logAdminActivity("update_study_note", "study_note"), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const [updated] = await db.update(studyNotesTable)
      .set({
        ...req.body,
      })
...
router.delete("/study-notes/:id", logAdminActivity("delete_study_note", "study_note"), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
