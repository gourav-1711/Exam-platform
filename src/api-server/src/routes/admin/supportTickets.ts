...
router.get("/support-tickets", async (req, res): Promise<any> => {
  try {
    const page = req.query.page
      ? routeParam(req.query.page as string | string[])
      : "1";
    const limit = req.query.limit
      ? routeParam(req.query.limit as string | string[])
      : "20";
    const status = req.query.status
      ? routeParam(req.query.status as string | string[])
      : "all";
    const search = req.query.search
      ? routeParam(req.query.search as string | string[])
      : "";
    ...
    const conditions = [];
    if (status !== "all")
      conditions.push(eq(supportTicketsTable.status, status as string));
...
router.get("/support-tickets/:id", async (req, res): Promise<any> => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch support ticket details" });
  }
});
...
router.post(
  ...
  async (req, res): Promise<any> => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to reply to support ticket" });
    }
  },
);
