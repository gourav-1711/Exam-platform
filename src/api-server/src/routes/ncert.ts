...
import { db, ncertBooksTable, questionsTable } from "@workspace/db";
...
router.get("/ncert-mcq/questions", async (req, res) => {
  try {
    const { classNum, subject, medium, page: pageStr } = req.query as Record<string, string>;
    const page = parseInt(pageStr) || 1;
    const limit = 10;
    const offset = (page - 1) * limit;

    let all = await db.select().from(questionsTable).where(eq(questionsTable.type, "ncert"));
...
router.get("/ncert-books", async (req, res) => {
  try {
    ...
    if (subject) all = all.filter(b => b.subject.toLowerCase() === subject.toLowerCase());
    if (medium) all = all.filter(b => b.medium.toLowerCase() === medium.toLowerCase());
    res.json(all);
  } catch (err) {
    req.log.error(err);
