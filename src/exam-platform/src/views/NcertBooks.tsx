...
import { useListNcertBooks, ListNcertBooksParams } from "@/lib/api";
  ...
  return (
    <PageTransition className="p-4 md:p-8 max-w-6xl mx-auto space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">Download or read NCERT textbooks online for free.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-4">
        {isLoading ? (
          ...
          books?.map((book) => (
            <Card key={book.id} className="card-hover border-border/50 rounded-2xl bg-card overflow-hidden flex flex-col">
              <div className="h-32 bg-gradient-to-br from-emerald-400 to-teal-600 p-4 flex items-end">
                <span className="text-white/90 font-bold tracking-widest text-2xl uppercase">NCERT</span>
              </div>
              <CardContent className="p-5 flex flex-col flex-1 gap-4 -mt-6">
                <div className="bg-card border shadow-sm rounded-xl p-3 flex-1 flex flex-col items-center text-center justify-center">
                  <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded mb-2">Class {book.classNum} • {book.subject}</span>
                  <h3 className="font-bold leading-tight">{book.title}</h3>
                  <p className="text-xs text-muted-foreground mt-1">{book.medium} Medium</p>
                </div>
